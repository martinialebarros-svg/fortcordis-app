...
st.sidebar.caption(f"Modo deploy (login em outro PC): {'sim' if is_modo_deploy() else 'não'}")
        ...
        with tab_importar:
            ...
            if arquivo_backup is not None:
                if st.button("🔄 Importar agora", key="btn_importar_backup", type="primary"):
                    ...
                    conn_local = None
                    try:
                        # Gravar backup em arquivo temporário em chunks (evita carregar todo o arquivo na memória e reduz risco de 502/timeout)
                        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as tmp:
                            tmp_path = tmp.name
                            chunk_size = 1024 * 1024  # 1 MB
                            total = 0
                            while True:
                                chunk = arquivo_backup.read(chunk_size)
                                if not chunk:
                                    break
                                tmp.write(chunk)
                                total += len(chunk)
