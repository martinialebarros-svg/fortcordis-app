...
st.sidebar.markdown("---")
st.sidebar.info("💡 Versão 2.0 - Sistema Integrado")
...
if menu_principal == "🏠 Dashboard":
    ...
    with col1:
        ...
        try:
            ...
            total = agend_hoje['total'].iloc[0] if not agend_hoje.empty else 0
        except:
            total = 0
    ...
    with col2:
        ...
        try:
            ...
            total = pend_conf['total'].iloc[0] if not pend_conf.empty else 0
        except:
            total = 0
    ...
    1. **Cadastrar Clínicas:** Vá em "🏢 Cadastros" para adicionar suas clínicas parceiras
    2. **Gerar Laudos:** Use "🩺 Laudos e Exames" como sempre fazia
    3. **Ver OS Automáticas:** Após gerar laudo, veja em "💰 Financeiro"
    """)
...
# ============================================================================
    ...
    # Abas do módulo
    tab_novo, tab_lista, tab_calendario = st.tabs(["➕ Novo Agendamento", "📋 Lista de Agendamentos", "📅 Calendário"])
    ...
    with tab_novo:
        ...
        with col2:
            ...
            clinica_agend = st.text_input("Clínica", key="novo_agend_clinica")
            ...
            for data in sorted(agendamentos_por_dia.keys()):
                ...
                with st.expander(f"📅 {data_fmt} - {len(agends_dia)} agendamento(s)"):
                    for agend in agends_dia:
                        status_icon = {"Agendado": "🟢", "Realizado": "✅", "Cancelado": "❌"}
...
# ============================================================================
    ...
    **Funcionalidades previstas:**
    - ✍️ Nova prescrição com cálculo automático
...
# ============================================================================
    ...
    try:
        ...
        if not contas.empty:
            contas['Valor'] = contas['Valor'].apply(lambda x: f"R$ {x:,.2f}")
    ...
    with tab_c1:
        ...
        with st.expander("➕ Cadastrar Nova Clínica", expanded=True):
            ...
            if st.button("✅ Cadastrar Clínica", type="primary"):
                if not novo_nome:
                    ...
                    # FORÇA O CAMINHO CORRETO DO BANCO
                    caminho_banco = Path(r"C:\Users\marti\Desktop\FortCordis_Novo\fortcordis.db")
        ...
        try:
            ...
            if not clinicas.empty:
                st.dataframe(clinicas, use_container_width=True, hide_index=True)
    ...
    with tab_c2:
        ...
        try:
            ...
            if not servicos.empty:
                servicos['Valor Base'] = servicos['Valor Base'].apply(lambda x: f"R$ {x:,.2f}")
                st.dataframe(servicos, use_container_width=True, hide_index=True)
            else:
                st.warning("⚠️ Execute o script inicializar_dados.py para popular os serviços")
        except:
            st.warning("⚠️ Execute o script inicializar_dados.py para popular os serviços")
        finally:
            conn.close()
